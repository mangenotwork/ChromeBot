...
func (p *Parser) parseStatement() ast.Statement {
	...
	utils.Debug("parseStatement = ", p.curTok)
...
func (p *Parser) parseSimpleStatement() ast.Statement {
	...
	utils.Debug("parseSimpleStatement = ", p.curTok)
...
func (p *Parser) parseLogicalOr() ast.Expression {
	...
	utils.Debug("parseLogicalOr = ", p.curTok)
...
func (p *Parser) parseTerm() ast.Expression {
	...
	p.enter()
	defer p.leave()
	utils.Debug("parseTerm =  ", p.curTok)
	expr := p.parseFactor()
	if expr == nil {
		return nil
	}
...
func (p *Parser) parseFactor() ast.Expression {
	...
	p.enter()
	defer p.leave()
	utils.Debug("parseFactor =  ", p.curTok)
	expr := p.parseUnary()
	if expr == nil {
		return nil
	}
...
func (p *Parser) parseUnary() ast.Expression {
	...
	utils.Debug("parseUnary =  ", p.curTok)
...
func (p *Parser) parsePrimary() ast.Expression {
	...
	utils.Debug("parsePrimary =  ", p.curTok)
...
func (p *Parser) parseCallOrIndex() ast.Expression {
	...
	utils.Debug("parseCallOrIndex: 开始")
...
func (p *Parser) parseIndex(left ast.Expression) ast.Expression {
	...
	utils.Debug("parseIndex: 开始解析下标，当前token:", p.curTok)

	// 跳过 [
	p.nextToken()
	utils.Debug("parseIndex: 跳过[后，当前token:", p.curTok)
	...
	utils.Debug("parseIndex: 索引表达式解析成功:", index)
...
func (p *Parser) parseChainCall(left ast.Expression) ast.Expression {
	...
	if existingChain, ok := left.(*ast.ChainCallExpr); ok {
		...
		if firstCall == nil {
			utils.Debug("parseChainCall: 无法创建第一个调用")
			return nil
		}
	...
	for p.curTokenIs(lexer.TokenDot) {
		utils.Debug("parseChainCall: 处理点号")
		...
		if methodCall == nil {
			utils.Debug("parseChainCall: 方法调用解析失败")
			return nil
		}
	...
	// 如果只有一个调用，直接返回它
	if len(chain.Calls) == 1 {
		utils.Debug("parseChainCall: 只有一个调用，直接返回")
		// 如果是虚拟调用，返回原始表达式
		if chain.Calls[0].Function.Name == "_value" {
			return chain.Calls[0].Args[0]
		}
...
func (p *Parser) createFirstChainCall(expr ast.Expression) *ast.CallExpr {
	...
	case *ast.Identifier: // 标识符（变量），创建虚拟调用
		return &ast.CallExpr{
			StartPos: v.Pos(),
	...
	case *ast.String, *ast.Integer, *ast.Float, *ast.Boolean: // 字面量，创建虚拟调用
		return &ast.CallExpr{
			StartPos: v.Pos(),
			Function: &ast.Identifier{
				StartPos: v.Pos(),
...
func (p *Parser) parseForOrForIn() ast.Statement {
	...
	for tokenCount < maxTokens && !p.curTokenIs(lexer.TokenEOF) {
		...
		if p.curTokenIs(lexer.TokenIn) {
			isForIn = true
			utils.Debug("parseForOrForIn: 发现 in 关键字，是 for...in 语句")
			break
		}

		// 检查是否遇到 {，说明是普通 for 循环
		if p.curTokenIs(lexer.TokenLBrace) {
			utils.Debug("parseForOrForIn: 发现 {，是普通 for 循环")
			break
		}
	...
	*p.lexer = savedLexer
	...
	// 4. 根据检测结果调用相应的解析函数
	if isForIn {
		utils.Debug("parseForOrForIn: 调用 parseForInStatement")
		return p.parseForInStatement()
	} else {
		utils.Debug("parseForOrForIn: 调用 parseForStatement")
...
func (p *Parser) parseWhileOrWhileIn() ast.Statement {
	...
	for tokenCount < maxTokens && !p.curTokenIs(lexer.TokenEOF) {
		...
		if p.curTokenIs(lexer.TokenIn) {
			isWhileIn = true
			utils.Debug("parseWhileOrWhileIn: 发现 in 关键字，是 while...in 语句")
			break
		}

		// 检查是否遇到 {，说明是普通 while 循环
		if p.curTokenIs(lexer.TokenLBrace) {
			utils.Debug("parseWhileOrWhileIn: 发现 {，是普通 while 循环")
			break
		}
	...
	*p.lexer = savedLexer
	...
	// 4. 根据检测结果调用相应的解析函数
	if isWhileIn {
		utils.Debug("parseWhileOrWhileIn: 调用 parseWhileInStatement")
		return p.parseWhileInStatement()
	} else {
		utils.Debug("parseWhileOrWhileIn: 调用 parseWhileStatement")
